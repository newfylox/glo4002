...
public class RequestTreatment {
	...
	private String sendingPostResquest(String urlWherePostRequestIsSent, String request) throws MalformedURLException, IOException{
	...
	private void sendStringRequestToEmergency(String requestToSend) throws IOException{
        OutputStreamWriter writer = new OutputStreamWriter(emergencyUrlConnector.getOutputStream());
        writer.write(requestToSend);
        writer.flush();
        writer.close();
	}
	
	private void preparePostRequestToEmergency() throws MalformedURLException, IOException {
		emergencyUrlConnector = (HttpURLConnection) new URL(EMERGENCY_URL).openConnection();
	...
	private String retreiveAnswerFromEmergency() throws IOException{
		BufferedReader reader = new BufferedReader(new InputStreamReader(emergencyUrlConnector.getInputStream()));
